...
run_zones_management_tests() {
    log_remote "INFO" "🧪 Starting Zones Management Tests"
    ...
    if is_test_failed; then
        log_remote "ERROR" "❌ Zones Management Tests: FAILED"
        return 1
    else
        log_remote "SUCCESS" "✅ Zones Management Tests: PASSED"
        return 0
    fi
}
